    ...
    * Olive - sample j is conditioned on the counterexample from sample j-1, so
    ...
    for data in per_prompt_data.values():
        ...
        data["passk_rates_noloop"] = [
            pass_at_k(n_samples, c_noloop, k) for k in k_values
        ] if n_samples else []
        data["passk_stds_noloop"] = [
            _passk_std(n_samples, c_noloop, k) for k in k_values
        ] if n_samples else []
    ...
    for k_idx in range(len(k_values)):
        ...
        m0, s0 = _mean_std([d["passk_rates_noloop"][k_idx]
                            for d in per_prompt_data.values()])
...
def print_passk_report(payload: Dict) -> None:
    ...
    k_max      = max(k_values)
    n_prompts  = config.get("n_prompts", len(per_prompt))
    has_noloop = len(agg.get("passk_rates_noloop", [])) == len(k_values)
    ...
    for pid in pid_list:
        ...
        r1     = rates[0]  * 100
        rk     = rates[-1] * 100
        if data.get("passk_rates_noloop"):
            b1 = data["passk_rates_noloop"][0]  * 100
            bk = data["passk_rates_noloop"][-1] * 100
