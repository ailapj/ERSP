...
def HasPulse (s : ArrestPatient) : Prop :=
  s.pulse = some Pulse.Present

def NoPulse (s : ArrestPatient) : Prop :=
  s.pulse = some Pulse.Absent

def VF (s : ArrestPatient) : Prop :=
  s.rhythm = some Rhythm.VentricularFibrillation

def PulselessVT (s : ArrestPatient) : Prop :=
  s.rhythm = some Rhythm.PulselessVT

def Asystole (s : ArrestPatient) : Prop :=
  s.rhythm = some Rhythm.Asystole

def PEA (s : ArrestPatient) : Prop :=
  s.rhythm = some Rhythm.PulselessElectricalActivity

def Shockable (s : ArrestPatient) : Prop :=
  VF s ∨ PulselessVT s

def NonShockable (s : ArrestPatient) : Prop :=
  Asystole s ∨ PEA s

def MonitorAttached (s : ArrestPatient) : Prop :=
  s.monitorAttached

def DefibrillatorAttached (s : ArrestPatient) : Prop :=
  s.defibrillatorAttached

def HasVascularAccess (s : ArrestPatient) : Prop :=
  s.vascularAccess ≠ VascularAccess.None

def AdvancedAirway (s : ArrestPatient) : Prop :=
  s.airway ≠ Airway.Basic

def EpinephrineGiven (e : Event) : Prop :=
  e = Event.EpinephrineGiven

def epiWindow : Trace.Interval :=
{
  lower := 180,
  upper := some 300,
  valid := by decide
...
-- epinephrine rule as MTL
def epiRule : Trace.Formula Event :=
  Trace.Formula.implies
    (Trace.Formula.atom EpinephrineGiven)
    (
      Trace.Formula.or
        (Trace.Formula.not (Trace.Formula.eventually
          unbounded
          (Trace.Formula.atom EpinephrineGiven)))
        (Trace.Formula.untilWithin
          epiWindow
          (Trace.Formula.not (Trace.Formula.atom EpinephrineGiven))
          (Trace.Formula.atom EpinephrineGiven))
    )
